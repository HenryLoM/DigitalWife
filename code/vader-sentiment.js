...
const vader = (() => {
    let B_INCR = 0.293    // Booster increment value
    let B_DECR = -0.293  // Booster decrement value
    const { abs, sqrt, log, pow } = Math;

    const constants = {
        C_INCR: 0.733,      // Capitalization emphasis increment (not used here)
        C_DECR: -0.733,    // Capitalization emphasis decrement (not used here)
        N_SCALAR: -0.74,  // Negation scalar
        NEGATE: [
            'aint',
            'arent',
            'cannot',
    ...
    function isNegated(word) {
        return constants.NEGATE.includes(word);
    }

    // Check if a word is a booster word and return its scalar
    function getBooster(word) {
        return constants.BOOSTER_DICT[word] || 0;
    }

    // Main function that calculates sentiment polarity scores
    function polarity_scores(text) {
        ...
        for (let i = 0; i < words.length; i++) {
            const word = words[i];
            let valence = constants.SENTIMENT_Lexicon[word] || 0;

            // Check for boosters in previous word
            if (i > 0) {
                const prevWord = words[i - 1];
                const booster = getBooster(prevWord);
                if (booster !== 0) {
                    valence += booster * (valence > 0 ? 1 : -1);
                }
            }

            // Check for negation in previous 3 words
            // (common heuristic in sentiment analysis)
